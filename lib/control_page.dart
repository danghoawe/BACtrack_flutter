...
import 'package:backtrack_test/main.dart';
...
class BACTestTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: bacTrackPlugin!.statusStream
          .where((status) => _bacTestStates.contains(status.state)),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        return ListTile(
          title: Text('BAC Test'),
          subtitle:
              Text(_snapshotToText(snapshot as AsyncSnapshot<BACtrackStatus>)),
  ...
  void initState() {
    ...
    _sub = widget.bacTrackPlugin!.statusStream.where((status) {
      ...
      if (status.state == BACtrackState.batteryVoltage) {
        setState(
            () => _voltage = 'Voltage: ${status.message.substring(0, 3)}V');
      } else if (status.state == BACtrackState.batteryLevel) {
        setState(() => _level = _levelToDescription(status.message));
      } else {
        setState(() =>
            _voltage = 'Error code: ${status.message}. Please try again.');
      }
    });
  }
...
class UseCountTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: bacTrackPlugin!.statusStream.where(
          (status) => UseCountTile._expectedStates.contains(status.state)),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        return ListTile(
...
class SerialNumberTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: bacTrackPlugin!.statusStream.where(
          (status) => SerialNumberTile._expectedStates.contains(status.state)),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        return ListTile(
...
class FirmwareVersionTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: bacTrackPlugin!.statusStream.where((status) =>
          FirmwareVersionTile._expectedStates.contains(status.state)),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        return ListTile(
